...
// OpenAI initialized lazily per-request
...
app.post('/api/generate-diagnostic', async (req, res) => {
  ...
  try {
    const prompt = `
Aja como um executivo sênior e consultor estratégico de vendas B2B da Signun. Você está falando diretamente com o empresário / CEO ativo chamado "${name}", dono ou líder da empresa "${company || 'sua empresa'}". Seu tom deve ser direto, profissional, altamente analítico e perspicaz. Você deve mostrar pontos cegos na operação que um empresário geralmente não vê.
...
3. Faça a ponte de como a Signun resolve isso através de UM ou MAIS dos nossos 3 pilares, dependendo da dor dele:
   - **Signun Performance:** Se o problema é falta de canais, funil bagunçado, falta de previsibilidade, falta de playbook estruturado.
   - **Signun Talent:** Se o problema é equipe fraca, turnover alto, recrutamento errado, falta de cultura comercial, necessidade de bons vendedores (hunters/closers).
   - **Signun Operação 360:** Se a empresa precisa de tudo, reestruturação completa end-to-end (Geralmente para quem tem muito gargalo junto ou tickets de alto valor precisando de profissionalização severa).
4. Termine com uma conclusão firme sobre o próximo passo óbvio que o CEO deve tomar e faça uma chamada sutil e elegante para ele avançar de fase e estruturar a máquina com a Signun.
...
`;
    ...
    const completion = await openai.chat.completions.create({
      model: "gpt-4o-mini",
      messages: [
        { role: "system", content: "Você é um C-level experiente em crescimento corporativo B2B e vendas complexas da Signun. Sem rodeios, sem emojis, altamente analítico e estratégico em HTML." },
...
app.post('/create-payment-intent', async (req, res) => {
  ...
  try {
    ...
    const intentOptions = {
      ...
      receipt_email: email,
      description: plan_type === 'pix' ? 'Formação em Vendas Consultivas Signun (Pix)' : 'Formação em Vendas Consultivas Signun (Cartão)',
      metadata: {
        customer_name: name,
        plan: plan_type
      }
